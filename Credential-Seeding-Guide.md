...
### Post-upgrade runtime reconfiguration (2.22.4 → 2.25.7)

After upgrading to n8n 2.25.7, review each live dynamic credential and pick
the resolver path that matches how the workflow is triggered:

| Credential family | Typical trigger | Post-upgrade resolver | Why |
| --- | --- | --- | --- |
| Microsoft Outlook / Teams / Graph | Webhook with bearer JWT | **Keep §10 OIDC** (`oidcSeedSource='oidc'`) | Login-time OBO + webhook lazy-seed populate `dynamic_credential_entry`; upstream Connect/cookie path does not replace unattended webhook runs |
| Monday.com (UI-driven, users click Connect) | Manual / editor | **Switch to N8N system resolver** | Upstream #30994 adds a **Connect** button that writes `dynamic_credential_user_entry` — simpler for interactive users |
| Monday.com (webhook, no per-user identity) | Webhook / schedule | **Keep §11 OAuth2 resolver** with `fallbackCredentialId` | Shared service account on miss; audit event `dynamic-credential-fallback-used` |

**Env-var cleanup.** If your deployment sets `N8N_SKIP_AUTH_ON_OAUTH_CALLBACK=true`
only to unblock dynamic-credential OAuth callbacks, **remove it** after upgrade —
upstream #31103 enables cookie auth for that path without the var. Keep it only
for embed/iframe OAuth on **static** credentials.

**Operator SQL (staging first).** Example — switch a Monday credential from §11
to the upstream system resolver (credential id is deployment-specific):

```sql
-- Unbind custom resolver; upstream system resolver is implicit
UPDATE credentials_entity
SET "resolverId" = NULL
WHERE id = '<monday-credential-id>';
```

Do **not** delete existing `dynamic_credential_entry` rows during transition —
if you revert to the §11 resolver, the per-subject tokens are still there.
...
### Upstream private-credential / manual-trigger warning (#31211)

> **Q:** My Outlook/Teams credential shows a *"private credential requires manual
> trigger"* warning in the node panel, but the webhook execution succeeds. Is
> something wrong?
>
> **A:** No. The warning ships with n8n 2.24+ (upstream
> [#31211](https://github.com/n8n-io/n8n/pull/31211)). It is a **false positive**
> for credentials seeded via the §10 OIDC Graph pipeline. The seed path runs at
> workflow execution time (login-time OBO or webhook lazy-seed) independently of
> the trigger type. The warning was added for the upstream `_user_entry` path,
> which does require a manual trigger until the user clicks **Connect** in the
> editor. §10-seeded credentials populate `dynamic_credential_entry` instead —
> unattended triggers work; ignore the NDV warning.
...
## Generic SaaS OAuth2 dynamic credentials (§11)

Cross-link: [CUSTOMS.md §11](./CUSTOMS.md#11-generic-saas-oauth2-dynamic-credentials)
is the upstream-rebase reference. This section is the operator-facing
walkthrough.

**Audience.** You're an n8n operator who wants a per-user OAuth2 credential
for a SaaS provider that is *not* Microsoft Graph — Monday.com, Slack,
Asana, Notion, your internal SaaS, etc. You've already wired up the §9
seeding endpoint or §10 OIDC self-seeding and want to add another
provider with minimal friction.

**TL;DR.** The same `oauth2` dynamic-credential resolver introduced in §9
and exercised against Microsoft Graph in §10 is already provider-agnostic.
What §11 adds:

1. An optional **`fallbackCredentialId`** field on the resolver config that
   lets webhook / chat / cron triggers fall back to a shared static
   credential when the per-user lookup misses (e.g., the request has no
   inbound bearer to derive identity from).
2. Always-on performance caches so large workflows (20+ nodes referencing
   the same credential) don't pay the DB+decrypt cost per node.
3. A new audit event `dynamic-credential-fallback-used` so you can answer
   "which workflows are running on the shared service account vs per-user
   tokens" from the log stream.

No new environment variables. No new endpoints. Just configure the
resolver and the credential the same way you'd configure §10.

### When to use this path vs. §10 OBO

| You want… | Use |
| --- | --- |
| Per-user Microsoft Graph (Outlook, Teams, OneDrive, SharePoint Online) | §10 OBO self-seeding |
| Per-user Azure OpenAI / non-Graph Microsoft API behind Entra | §10 OBO self-seeding with the right audience |
| Per-user **any non-Microsoft SaaS** with a standard authorization-code OAuth2 flow | §11 (this section) |
| Server-to-server / app-only credentials | §9 `/seed` endpoint with an externally-acquired token |
| One shared service-account credential for all workflows, no per-user identity | Plain static OAuth2 credential, no resolver |

### Setup — Monday.com walkthrough

This is the concrete example. Slack / Asana / Notion follow the same shape
— see [Template configs](#template-configs-slack-asana-notion) below.

#### Prerequisites

- A Monday.com OAuth2 app — Monday Developer Center → "Create a new app" →
  add the OAuth integration. Note the **Client ID** and **Client Secret**.
- The app's "OAuth & Permissions" tab has the following redirect URI:
  `https://<your-n8n-host>/rest/oauth2-credential/callback`
- The dynamic-credentials feature flag is on:
  `N8N_ENV_FEAT_DYNAMIC_CREDENTIALS=true`. (You already enabled this if §9
  or §10 are working.)

#### Step 1 — Register the resolver

n8n editor → **Settings → Credential Resolvers → New resolver**

| Field | Value |
| --- | --- |
| Name | `Monday OAuth2 resolver` |
| Type | `oauth2` |
| Metadata URI | _(see note below)_ |
| Client ID | _your Monday client id_ |
| Client Secret | _your Monday client secret_ |
| Subject Claim | `sub` |
| Validation Method | `UserInfo` (Monday returns a verifiable user object at its userinfo equivalent) |
| Fallback Credential ID | _leave empty for now; we add this in Step 4 if needed_ |

**On Metadata URI.** Monday doesn't publish a `.well-known/openid-configuration`
document. The Validation dropdown still expects a URL to satisfy the
config schema; in practice, when you use the **Token Introspection** or
**UserInfo** validation methods n8n will use the resolver's configured
introspection / userinfo endpoint, not the metadata document. If your
provider has no discovery URL at all, point Metadata URI at the provider's
home page (`https://auth.monday.com/`) — it's parsed as a URL but only
used as a fallback for endpoint discovery. For providers with a discovery
URL (Slack, Notion), use the real one.

Click **Save**.

#### Step 2 — Create the credential

n8n editor → **Credentials → New credential → Monday.com OAuth2 API**

| Field | Value |
| --- | --- |
| Name | `Monday account (per-user)` |
| OAuth Redirect URL | _shown by n8n; copy it into the Monday app config in Step 0_ |
| Client ID | _same as resolver_ |
| Client Secret | _same as resolver_ |
| Auth URL | `https://auth.monday.com/oauth2/authorize` |
| Token URL | `https://auth.monday.com/oauth2/token` |
| Scope | _whatever your workflow needs; e.g._ `me:read boards:read boards:write` |
| Resolvable | **on** |
| Resolver | `Monday OAuth2 resolver` (from Step 1) |

Click **Save**.

> **Post-2.25.7 note.** If users will connect via the n8n editor (manual
> trigger, UI-driven workflows), prefer the **upstream N8N system resolver**
> and the editor **Connect** button (#30994) instead of this §11 walkthrough.
> The steps below remain the right path for webhook/backend integrators who
> need `fallbackCredentialId` or subject-keyed `dynamic_credential_entry` rows.

#### Step 3 — Wire it into a workflow

Add a Monday node to a workflow, point it at the credential from Step 2.
Optionally set the workflow's **Settings → Credential resolver** to the
Monday resolver as well — this widens the discovery surface so the
seeding paths see the workflow's credentials too. (Same mechanism as
§10's "workflow-level binding".)

#### Step 4 — First execution as a real user

The first time a user triggers this workflow (UI or webhook) the resolver
will look for a per-user entry, find none, and:

- **Without `fallbackCredentialId` (default)** — return a
  `CredentialResolverDataNotFoundError`. The editor surfaces an "Authorize
  with Monday" button; user clicks once, Monday's consent screen appears,
  user clicks "Authorize", returns to n8n with their per-user token now
  persisted in `dynamic_credential_entry`. All subsequent runs *for that
  user* are silent.
- **With `fallbackCredentialId` set** — return the shared static
  credential's data. The workflow runs against the shared account. n8n
  also emits a `dynamic-credential-fallback-used` audit event so you can
  see which executions used the shared account.

```mermaid
flowchart TD
    Trigger[Workflow trigger] --> Resolve["Resolver.getSecret(credentialId, context)"]
    Resolve --> Identifier{"Identifier can extract subject?"}
    Identifier -->|"yes"| Lookup["Lookup dynamic_credential_entry by (credentialId, subject)"]
    Identifier -->|"no"| Fallback?
    Lookup --> Hit?{Per-user data found?}
    Hit? -->|"yes"| Return["Return per-user token"]
    Hit? -->|"no"| Fallback?{"fallbackCredentialId configured?"}
    Fallback? -->|"yes"| LoadFallback["Load credential by id, decrypt, emit dynamic-credential-fallback-used"]
    Fallback? -->|"no"| Throw["Throw CredentialResolverDataNotFoundError → editor shows 'Authorize with Monday'"]
    LoadFallback --> Return
```

### Hybrid fallback — when to use it

The fallback is for triggers that legitimately have no per-user identity:

| Trigger type | Per-user works? | Fallback usage |
| --- | --- | --- |
| Editor UI / manual execute as authenticated user | yes — uses the n8n session | rare; user has clear identity |
| Webhook + `BearerTokenExtractor` (user-bearer mode) | yes — identifier extracts subject from inbound JWT | rare |
| Webhook with **no** identity extractor (public/machine webhook) | no — context has no identity | **primary use case** |
| Chat trigger from an anonymous channel | no | **primary use case** |
| Cron / interval trigger | no | **primary use case** |
| Workflow triggered by another workflow via "Execute Workflow" node | depends on parent's context | usually the parent's identity propagates; fallback as safety net |

**Anti-pattern.** Don't use fallback as the *default* path for a workflow
that *should* be per-user — you'll lose audit trail (every execution
looks shared) and you'll mask consent issues (per-user OAuth never gets
fixed because the fallback hides it). Use it explicitly for the trigger
types in the right column above.

### Template configs (Slack, Asana, Notion)

The resolver setup is identical except for the endpoints and scopes.

**Slack.**

| Field | Value |
| --- | --- |
| Metadata URI | `https://slack.com/.well-known/openid-configuration` |
| Validation Method | `JWT Claim` (Slack issues JWT-shaped access tokens for its OIDC scopes) |
| Auth URL (on credential) | `https://slack.com/oauth/v2/authorize` |
| Token URL (on credential) | `https://slack.com/api/oauth.v2.access` |
| Scope | `chat:write users:read` (or whatever) |

**Asana.**

| Field | Value |
| --- | --- |
| Metadata URI | `https://app.asana.com/api/1.0/openid_connect/well-known` |
| Validation Method | `UserInfo` |
...
| Field | Value |
| --- | --- |
| Metadata URI | _(none — use Notion's API base, `https://api.notion.com/`)_ |
| Validation Method | `UserInfo` |
| Auth URL | `https://api.notion.com/v1/oauth/authorize?owner=user` |
| Token URL | `https://api.notion.com/v1/oauth/token` |
| Scope | _(Notion uses workspace-scoped grants, not OAuth scope strings — leave empty)_ |

These configs are **not** verified end-to-end in this commit — they're
template starting points. Verify against the provider's current OAuth
documentation before relying on them.

### Performance notes

§11 added three caches; they are always-on, no toggle.

- **PE-1+PE-2.** Resolver entity + decrypted config cached for 60s per
  `resolverId`. Invalidated explicitly when the admin updates or deletes
  the resolver. Impact: in a 20-node workflow all hitting the same
  Monday credential, only the first node does the DB query + cipher
  decrypt; the other 19 are cache hits.
- **PE-3.** The decrypted credential context (`executionContext.credentials`)
  is memoized via a non-enumerable Symbol on the `executionContext`
  object itself. Scope is per-execution; never crosses execution
  boundaries; never serialized. Impact: in a 20-node workflow, the
  context is decrypted exactly once.

If you see stale resolver config after an admin edit, you can:
- Wait up to 60s for the TTL fallback (no action needed), or
- Restart the n8n process to drop the in-memory cache, or
- Verify the admin edit landed in the DB (`SELECT id, "updatedAt" FROM
  dynamic_credential_resolver WHERE id = '<resolver-id>';`) — if it
  shows a fresh timestamp, the explicit invalidation hook should have
  fired and the next resolution will be uncached.

### Audit observability

Every fallback use emits this audit event:

| Event | `n8n.audit.dynamic-credential.fallback-used` |
| --- | --- |
| Payload | `{ credentialId, resolverId, fallbackCredentialId, subject? }` |
| Token material? | **No** — only ids and the resolved subject (or `undefined` when the identifier itself failed) |
| Streaming | Reaches the standard log-streaming pipeline alongside §10's `oidc-graph-token-*` events |

To answer "which executions ran on the shared service account":

```sql
-- pseudocode for your log-stream sink
SELECT timestamp, payload->>'credentialId' AS cred, payload->>'subject' AS user_subject
FROM events
WHERE event_name = 'n8n.audit.dynamic-credential.fallback-used'
ORDER BY timestamp DESC;
```

A high volume of these events for a credential where you *expected*
per-user identity is the signal to inspect: either users aren't
completing their consent dance, or a webhook node is missing its
`BearerTokenExtractor`, or the wrong trigger type was wired up.

### Troubleshooting

| Symptom | Likely cause | Fix |
| --- | --- | --- |
| Editor shows "Authorize with Monday" on every execution, never persists | Resolver isn't set on either the credential or the workflow settings | Set on at least one. Both is fine; credential-level wins on collision. |
| "Failed to resolve dynamic credentials … UserInfo query failed" | Provider's userinfo endpoint rejects the access token (wrong audience, expired) | Switch the resolver's Validation Method to `JWT Claim` if the token is a JWT, or `Token Introspection` if the provider supports it |
| Fallback always returns even though the user clearly authorized | Two different credential rows; the dance stored the token against credential A but the workflow uses credential B | The user's per-user entry is keyed by `(credentialId, subject)`. Same subject + different credentialId = different rows. Either delete the duplicate credential or have the user re-authorize against the right one |
| Admin changed the resolver config but workflows still use the old one for ~1 min | Resolver-entity cache hasn't been invalidated (e.g., the admin edit happened via direct DB write that bypasses the service) | Wait 60s for TTL fallback, or restart n8n. Edits via the n8n UI / API always trigger immediate invalidation |
| `dynamic-credential-fallback-used` event missing the `subject` field | The identifier itself failed (no inbound JWT to extract from) — fallback was triggered as a safety net | This is expected for machine webhooks and anonymous chat. The event still records the credential and resolver ids; you just don't get a user attribution |

### What's deliberately out of scope for v1

| Item | Why | Where to find it |
| --- | --- | --- |
| Proactive "Connect" banner for UI-driven SaaS credentials | **Shipped upstream (#30994)** for the N8N system resolver path | Use system resolver + Connect for editor users; keep §11 for webhook/backend paths |
| Refresh-token concurrency control | The race exists for static OAuth2 credentials in upstream too; DB writes are atomic so no corruption; worst case is one transient error per race storm | See [CUSTOMS.md §11 — Refresh-token concurrency](./CUSTOMS.md#11-generic-saas-oauth2-dynamic-credentials) |
| Per-resolver enable/disable toggle | Resolvers can already be deleted or have their `oidcSeedSource` cleared. Adding an extra "enabled" boolean would just be another knob | Out — use existing controls |
| Telemetry on cache hit rate | Useful but cheap to add later; the audit-event coverage is what answers "is the system being used correctly" | Out — file an issue if you want it |
