...
**Changed in 2.34.6 — Azure AD `roles`-array handling removed.** The original
`337ab0a67c "handle Azure AD roles array claim format"` logic (normalizing a
`roles: [...]` claim to a single instance role) was **intentionally dropped** this
upgrade. Roles-array claims are now expected to be mapped via **expression-based
role mapping** (`scopesUseExpressionMapping`), which is strictly more flexible.
Do **not** re-introduce array normalization on a future rebase. Operational
caveat: an instance still on *direct-claim* provisioning whose IdP emits `roles`
as an array must enable expression mapping (and set `defaultInstanceRole`
appropriately) — otherwise a previously-assigned elevated role can go stale on the
next login instead of being downgraded. Fail-closed login gating
(`assertSsoLoginAllowed` + `BLOCK_ACCESS_ASSIGNMENT`) is unaffected; there is no
external-attacker elevation path.
...
**Commit on current branch**
- `71862d2f42 feat(dynamic-credentials): resolve private OAuth2 credentials on manual test-webhook/chat runs + supply-data execution-context fallback` (cherry-picked from `6aa25eb000`)
...
**Commit on current branch**
- `d84faf067b feat(Webhook Node): support multiple public keys for JWT auth` (cherry-picked from `1296158485`)
