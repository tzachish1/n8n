...
3. **Cherry-pick customizations in order.** From the current baseline branch
   (`feat/upgrade-to-n8n-2.20.7-exp.0` at time of writing), cherry-pick these
   **eight** commits one at a time in this exact order:

   ```bash
   git cherry-pick 5d11c529c1   # node governance
   git cherry-pick 147edb3b68   # external secrets (Akeyless)
   git cherry-pick 3a5eb1514a   # SSO OIDC provisioning hardening
   git cherry-pick 1512768c3d   # Azure OpenAI APIM (nodes-langchain)
   git cherry-pick a483fff476   # Prometheus labels + Docker build splits
   git cherry-pick bae30c2377   # CI workflow trim (Section 8)
   git cherry-pick 575dc51611   # upgrade chore (build/test/lint mechanical fixes)
   git cherry-pick $(git log --format=%H --grep='docs(upgrade)' -1 feat/upgrade-to-n8n-2.20.7-exp.0)  # this docs file
   ```
...
**Recorded findings (2.20.7-exp.0, against `n8n@2.20.7-exp.0` =
`1bb97d8392`).** All 8 customizations remained required:
...
- §3 Login-event emits — partial overlap. Upstream now emits
  `user-login-failed` from `authenticateWithPassword` (wrong-credentials
  path) and `user-logged-in` from the standard login flow. Upstream does
  **not** emit either from `validateSsoRestrictions` (our SSO-blocked
  path) or from the OIDC controller. Our emits remain orthogonal,
