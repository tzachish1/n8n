...
| `feat/upgrade-to-n8n-2.26.8`             | `n8n@2.26.8` | 11 | dynamic-credentials areas (§9, §10, §11) joined the cherry-pick set. **§3 partially adopted upstream:** upstream now ships `scopesInstanceRoleClaimName` in `sso.config.ts` *and* the `user-logged-in` emit in `oidc.controller.ee.ts` — the fork emit was dropped to avoid double-counting (see the §3 sub-section). §10 migration renumbered to `…029` after a timestamp collision with upstream's new common migrations. §11 redaction hunks **dropped**: upstream independently implemented `executedByUserId`-based `isOwnDynamicCredentialsExecution`, a superset of the fork's manual-only rule. |
...
- `34bf631218c feat(node-governance): introduce node governance with audit, enforcement, and per-project overrides`
...
- `packages/cli/src/workflow-runner.ts` *(runtime enforcement — applies to **all**
  execution modes, including schedule/webhook/trigger runs that carry no `userId`;
  the guard turns only on a resolvable project. Keep it that way: gating on
  `data.userId` silently exempts every automated run, so a policy added after a
  workflow was activated would never apply to it. Pinned by the
  `node governance enforcement` tests in `src/__tests__/workflow-runner.test.ts`.)*
...
**Commit on current branch (single, squashed)**
- `899a0d319b8 feat(external-secrets): add Akeyless provider with subfolder support and log redaction`
...
- **⚠️ `axios` must stay a runtime `dependency` of `packages/cli` (burned us on 2.34.6).**
  `akeyless.ts` imports `axios` at top level (loaded at boot when external-secrets
  providers register), and §11's `oauth2-jwt-claim-identifier.ts` imports it too.
  Upstream 2.34.6 demoted `axios` to `devDependencies` (their cli runtime no longer
  uses it), so the pruned production Docker deploy (`pnpm --prod deploy`) dropped it
  and the image crash-looped on boot with `Cannot find module 'axios'`. On every
  rebase, confirm `axios` is under `"dependencies"` (not `"devDependencies"`) in
  `packages/cli/package.json` and re-run `pnpm install` so the lockfile records it as
  a prod dep. Audit: `node -e "const p=require('./packages/cli/package.json'); process.exit(p.dependencies?.axios?0:1)"`.
...
- Role mapping from Azure App Roles (`roles` claim, not `n8n_instance_role`) —
  **single-value only**; array-valued claims are upstream's job, see the note below.
...
- **Userinfo → ID-token-claims fallback** so a login isn't failed when Entra's
  Graph-hosted userinfo endpoint rejects an api-audience access token. See
  "Userinfo fallback" below — this one was silently dropped once already.
...
**Changed in 2.34.6 — Azure AD `roles`-array handling removed. Re-verified on
2.35.7; this is settled, not provisional.** The original
`337ab0a67c "handle Azure AD roles array claim format"` logic (normalizing a
`roles: [...]` claim to a single instance role) was **intentionally dropped** on
the 2.34.6 upgrade. Roles-array claims are handled through upstream's
**expression-based role mapping** (`scopesUseExpressionMapping` /
`N8N_SSO_SCOPES_USE_EXPRESSION_MAPPING`), which is strictly more flexible.
Do **not** re-introduce array normalization on a future rebase.

**Userinfo fallback — MUST survive every rebase (already lost once).**
`OidcService.loginUser` and `processTestCallback` both call
`fetchUserInfo`. On Azure Entra the userinfo endpoint *is* Microsoft Graph
(`https://graph.microsoft.com/oidc/userinfo`) and only accepts Graph-audienced
tokens. With direct-claim provisioning enabled, `buildAuthorizationScope()`
appends the `api://<n8n-app>/…` provisioning scope, so Entra issues an access
token whose `aud` is the n8n API and Graph answers `401 WWW-Authenticate`
(the challenge names client id `00000003-0000-0000-c000-000000000000`, i.e.
Graph). Upstream turns that into `BadRequestError('Invalid token')` and **all
SSO logins fail**. The ID token still carries `email`/`name`/`sub`, so both
paths fall back to `userInfoFromIdTokenClaims()` and only throw when there is
no email claim.

Note the coupling to §10: the api-audience access token is *required* as the OBO
assertion for Graph auto-seeding, so "just disable direct-claim provisioning"
trades one feature for another. Keep the fallback.

History: present 2.25.7 → 2.32.6, **silently dropped in the 2.34.6
customization re-squash**, shipped broken in 2.35.7 and only surfaced in a
cluster where the provisioning flags were on. Restored with tests
(`falls back to ID token claims …` in `oidc.service.ee.test.ts`).

```bash
# Fork fallback must be present in BOTH userinfo call paths.
# Expect 3 — the helper definition plus one call in loginUser and one in
# processTestCallback. Anything less means a rebase dropped a call site.
git grep -c 'userInfoFromIdTokenClaims' HEAD -- \
  packages/cli/src/modules/sso-oidc/oidc.service.ee.ts
```

Verified present upstream at `n8n@2.35.7` (re-run these on every upgrade):

```bash
# Upstream expression mapping must exist (non-zero hits) …
git grep -nE 'scopesUseExpressionMapping|isExpressionMappingEnabled|provisionExpressionMappedRolesForUser' \
...
# … and the retired fork array branch must NOT come back (expect 0 hits)
git grep -nE 'Array\.isArray\(roleSlugInput\)' HEAD -- packages/cli/src/modules/provisioning.ee/
```

Operational caveat: an instance still on *direct-claim* provisioning whose IdP
emits `roles` as an array must enable expression mapping (and set
`defaultInstanceRole` appropriately) — otherwise a previously-assigned elevated
role can go stale on the next login instead of being downgraded. Fail-closed
login gating (`assertSsoLoginAllowed` + `BLOCK_ACCESS_ASSIGNMENT`) is
unaffected; there is no external-attacker elevation path.

**Commit on current branch (single, squashed)**
- `97b8eaf6a73 feat(sso-oidc): harden Azure Entra direct-claim provisioning for instance and project roles`

**Original component commits (pre-squash)**
- `337ab0a67c` handle Azure AD `roles` array claim format — **DROPPED on 2.34.6.
  Superseded by upstream expression-based role mapping. Do NOT re-pick.**
...
#### Audit / log-streaming login events (2026-04-29; FULLY absorbed upstream as of 2.26.8)
...
**Status: both halves are now upstream. Nothing here is fork code any more.**

- The **failed-password-under-SSO** half was absorbed in 2.22.4 (PR
  n8n-io/n8n#29856 added the `user-login-failed` emit in the SSO-blocked path);
  our `auth.controller.ts` hunk was dropped from the §3 commit during
  cherry-pick (resolved by keeping HEAD).
- The **successful-OIDC-login** half was absorbed in **2.26.8** — upstream added
  its own `eventService.emit('user-logged-in', …)` in `oidc.controller.ee.ts`.
  The fork emit was dropped then to avoid double-counting. Re-verified on
  2.34.6 and 2.35.7: `oidc.controller.ee.ts:178` is byte-identical between HEAD
  and the upstream tag, and there is exactly **one** emit site.

Both fork shapes are preserved below purely as re-application recipes in case
upstream ever regresses. **Do not re-introduce either one while upstream still
emits** — a second emit double-counts every login in the SIEM.

1. **Failed password login under SSO** *(absorbed upstream in 2.22.4)* —
   `validateSsoRestrictions` in `auth.controller.ts` threw an `AuthError`
   *before* the existing `user-login-failed` emit was reached. Every blocked
   attempt (including brute-force against the owner account, which is the only
   one that *can* still log in with email+password while SSO is enabled) was
   dropped on the floor. Upstream PR n8n-io/n8n#29856 landed an equivalent
   emit; the §3 commit no longer carries the `auth.controller.ts`
   parameter-rename hunk.
2. **Successful OIDC login** *(absorbed upstream in 2.26.8)* —
   ...
   already emitted the same event, so this was pure parity. Upstream has since
   implemented it at the same point in `callbackHandler()`.

The relay (`packages/cli/src/events/relays/log-streaming.event-relay.ts`)
maps both events to `n8n.audit.user.login.failed` /
`n8n.audit.user.login.success`.

**Entry points / key files** *(both now upstream-owned — recorded fork shapes
are for re-application only if upstream regresses)*
- `packages/cli/src/controllers/auth.controller.ts` — **upstream since 2.22.4.**
  Recorded fork shape: `validateSsoRestrictions(preliminaryUser,
  emailOrLdapLoginId)` took an extra parameter and emitted `user-login-failed`
  with `{ authenticationMethod: 'email', userEmail, reason: 'SSO is enabled' }`
  before `throw new AuthError(...)`; the call site passed `emailOrLdapLoginId`
  through. Upstream PR n8n-io/n8n#29856 landed an equivalent emit on a slightly
  different code path.
- `packages/cli/src/modules/sso-oidc/oidc.controller.ee.ts` — **upstream since
  2.26.8.** Recorded fork shape: constructor injects `EventService`;
  `callbackHandler()` emits `user-logged-in` with
  `{ user, authenticationMethod: 'oidc' }` after `issueCookie`, before
  `res.redirect('/')`. Upstream's implementation sits at the same point.
  **Cherry-pick hazard:** when §3 carried this emit, git's textual auto-merge
  repeatedly doubled the constructor-injected `eventService` parameter against
  upstream's own injection (TS2451 + TS2554). See the troubleshooting row
  "Constructor-DI auto-merge doubling after cherry-pick…".

**Audit events emitted**

| Event name (audit)                | Trigger                                                    | Payload extras                            | Source as of 2.35.7                       |
|-----------------------------------|------------------------------------------------------------|-------------------------------------------|-------------------------------------------|
| `n8n.audit.user.login.failed`     | password login while SSO enabled and user not allowed      | `userEmail`, `reason: "SSO is enabled"`   | upstream (PR n8n-io/n8n#29856, since 2.22.4) |
| `n8n.audit.user.login.success`    | successful OIDC callback                                   | `user`, `authenticationMethod: "oidc"`    | upstream (`oidc.controller.ee.ts`, since 2.26.8) |

**Upgrade checklist**
- **Both emits are upstream.** On every upgrade, verify upstream *still* emits
  them — `user-login-failed` from the SSO-blocked branch in
  `auth.controller.ts`, and `user-logged-in` from `oidc.controller.ee.ts`. The
  adoption-audit block at the bottom of this file greps for both.
- **Assert exactly one emit site** in `oidc.controller.ee.ts`. More than one
  means a rebase re-introduced the retired fork emit; delete the fork copy.
- Only if upstream **regresses** (0 hits), re-apply the corresponding fork
  shape recorded under "Entry points / key files" above — for OIDC, at the
  equivalent point in `callbackHandler()`: **after `issueCookie`**, before
  redirect.
- The relay listens for both event names already (`log-streaming.event-relay.ts:63-64`);
  if upstream changes the event name or payload shape, update the relay handler.
...
- `e544d3e8367 feat(nodes-langchain): add Azure API Management (APIM) support for Azure OpenAI`
...
**Commit on current branch (single, squashed)**
- `948b1c5d412 feat(core): add execution_mode and project_id labels to Prometheus metrics plus Docker build splits`
...
**Smoke-testing these metrics — read before concluding the labels regressed.**
Every label is independently flag-gated and defaults to **false**, so a scrape
showing `n8n_audit_workflow_executed_total 1` with *no* labels is the expected
output of a misconfigured smoke, not a dropped customization. The counters are
also created lazily on first event, so an empty `/metrics` proves nothing — you
must actually execute a workflow. Minimum env for a full-label smoke:
...
N8N_METRICS_INCLUDE_EXECUTION_MODE_LABEL=true
N8N_METRICS_INCLUDE_PROJECT_ID_LABEL=true
N8N_METRICS_INCLUDE_NODE_TYPE_LABEL=true
```

Note also that only `n8n.audit.workflow.executed` receives the full label set
(`buildWorkflowLabels`); the other `n8n.audit.workflow.*` events intentionally
get workflow id/name only (`buildBaseWorkflowLabels`).
...
**Upstream #31211 interaction (n8n 2.24+, editor-only).** Upstream PR
[#31211](https://github.com/n8n-io/n8n/pull/31211) adds an NDV warning when
a *private* (`isResolvable`) credential is bound to a workflow whose active
trigger is not a manual trigger (`useNodeHelpers.ts` →
`collectPrivateCredentialIssues` / `workflowHasIncompatibleTrigger`). That
warning targets the upstream `_user_entry` / editor "Connect" path, which
genuinely requires an interactive login before unattended triggers can resolve
per-user tokens.

The fork does **not** forward-port a runtime manual-trigger gate onto
`WorkflowValidationService` — doing so would block §10 lazy-seed and §9
programmatic seed on webhook/schedule triggers that succeed today. The fork's
existing publish-time check (`validateDynamicCredentials`) is orthogonal: it
requires a trigger with identity-establishment hooks configured, not a manual
trigger specifically.

**Expected false positive:** Outlook/Teams credentials seeded via the §10 OIDC
Graph pipeline (login-time OBO or webhook lazy-seed) may still show *"private
credential requires manual trigger"* in the NDV while webhook execution
succeeds. Cosmetic only — no runtime impact. See
[Credential-Seeding-Guide.md — Part 5](./Credential-Seeding-Guide.md#upstream-private-credential--manual-trigger-warning-31211).
...
| --- | --- | --- |
| `N8N_SKIP_AUTH_ON_OAUTH_CALLBACK` | **Obsolete for dynamic-credential OAuth callbacks** since upstream **#31103** (n8n 2.23.0). Cookie-based callback auth now works without this var for `origin=dynamic-credential` flows. | **Remove from deployment configs** (`docker-compose*`, K8s manifests, `.env`) if it was only set to unblock dynamic-credential Connect/seed. **Keep** (`=true`) only when you still need embed/iframe OAuth for **static** credentials — upstream still gates that path on this var. Repo `grep` (Phase 5, 2.25.7 upgrade): zero references in tracked `docker-compose*` / `.env*` files. |
...
### 11. Generic SaaS OAuth2 Dynamic Credentials

See also: [Credential-Seeding-Guide.md — Generic SaaS OAuth2 dynamic
credentials](./Credential-Seeding-Guide.md#generic-saas-oauth2-dynamic-credentials-11)
for the operator-facing setup walkthrough. This section is the
upstream-rebase reference.

**What & why.** §9 + §10 established per-user dynamic OAuth credentials for
Microsoft Graph. The same machinery is — by design — IdP-agnostic: any
OAuth2 provider with a standards-compliant authorization-code flow can
drive it (Monday.com, Slack, Asana, Notion, custom internal SaaS, etc.).
§11 closes the gaps that surfaced when actually exercising the path against
a non-Microsoft provider:

1. **Hybrid fallback** (`fallbackCredentialId`). Webhook triggers and chat
   triggers often have no per-user identity to key on. Without a fallback
   the resolver throws and the workflow appears broken to operators who
   set up the resolver thinking "shared service account is fine for these
   paths". The fork now allows the admin to attach an optional fallback
   credential id to the resolver config; on a per-user miss, the resolver
   returns the decrypted data of that shared static credential and emits
   `dynamic-credential-fallback-used` so the use is observable.
2. **Performance hardening.** A profile audit before shipping the
   non-Microsoft path showed three hot-loop inefficiencies that don't
   matter for a 1-2 node Graph workflow but become measurable on
   20-30 node workflows (each node resolves its credential separately):
   resolver entity loaded from DB on every credential resolution, the
   encrypted config decrypted and JSON-parsed on every resolution, the
   execution-context credentials decrypted on every resolution within the
   same execution. All three are now cached (60s TTL on the cross-call
   resolver cache, per-execution memoization on the context decrypt).

The §10 OBO flow remains the recommended path for Microsoft Graph
specifically because Entra rejects the standard `/authorize` dance for
mixed-audience scopes; non-Microsoft providers don't have that constraint
and use the boring upstream authorization-code grant directly.

**Constraints.** §11 deliberately ships **zero new environment variables.**
The hybrid fallback is per-resolver config (in the encrypted DB row), the
caches are always-on (no toggle needed), and the audit event piggybacks on
the existing `eventService` infrastructure. The §10 env-var inventory
...
*Backend (modified)*
- `packages/cli/src/modules/dynamic-credentials.ee/credential-resolvers/identifiers/oauth2-utils.ts`
  *(adds `fallbackCredentialId` to the shared `OAuth2OptionsSchema`)*
- `packages/cli/src/modules/dynamic-credentials.ee/credential-resolvers/oauth-credential-resolver.ts`
  *(new `loadFallbackOrThrow` helper, two-step lookup in `getSecret`,
  injects `CredentialsRepository` + `EventService`, surfaces the new
  field in the admin UI dropdown)*
- `packages/cli/src/modules/dynamic-credentials.ee/services/dynamic-credential.service.ts`
  *(adds `loadResolverEntityCached` + `invalidateResolverEntityCache`,
  uses cached resolver in `resolveIfNeeded`, memoizes
  `buildCredentialContext` per execution via Symbol key, injects
  `CacheService`)*
- `packages/cli/src/modules/dynamic-credentials.ee/services/credential-resolver.service.ts`
  *(calls the new invalidation hook on `update` and `delete`)*
- `packages/cli/src/events/maps/relay.event-map.ts`
  *(declares `dynamic-credential-fallback-used` event type)*
- `packages/cli/src/events/relays/log-streaming.event-relay.ts`
  *(routes the new event to `n8n.audit.dynamic-credential.fallback-used`)*

*Tests (new + modified)*
- `packages/cli/src/modules/dynamic-credentials.ee/credential-resolvers/__tests__/oauth-credential-resolver.test.ts`
  *(8 new fallback test cases; updated constructor calls for the 2 new
  injected deps; verified locally — 40/40 pass)*
- `packages/cli/src/modules/dynamic-credentials.ee/services/__tests__/dynamic-credential.cache.test.ts`
  *(new file — PE-1/PE-2/PE-3 behavior coverage; uses relative imports
  to dodge the pre-existing `@/`-mapping issue that blocks
  `dynamic-credential.service.test.ts` from loading locally)*
- `packages/cli/src/modules/dynamic-credentials.ee/services/__tests__/dynamic-credential.service.test.ts`
  *(updated all 5 `new DynamicCredentialService(...)` call sites to pass
  the new `CacheService` arg)*
- `packages/cli/src/events/__tests__/log-streaming-event-relay.test.ts`
  *(1 new audit-event test)*

*Documentation*
- `Credential-Seeding-Guide.md` *(new top-level §11 section)*
- `CUSTOMS.md` *(this entry)*

**Runtime contract**

| Surface | Value | Notes |
| --- | --- | --- |
| Env vars | _none new_ | Discipline preserved — see "Constraints" above |
| Resolver config (DB) | adds optional `fallbackCredentialId: string` | Empty string treated as unset; UI default is empty |
| Cache key (resolver) | `dynamic-credentials:resolver-entity:<resolverId>` | 60s TTL; invalidated explicitly on `update`/`delete` |
| Cache key (per-exec context) | `Symbol.for('n8n.dynamicCredentials.memoizedCredentialContext')` | Attached as non-enumerable property; never serialized |
| Audit event | `dynamic-credential-fallback-used` | Payload: `{ credentialId, resolverId, fallbackCredentialId, subject? }` — no token material |
| Audit-bus event name | `n8n.audit.dynamic-credential.fallback-used` | Log-streaming relay path |

**Refresh-token concurrency.** Verified during the §11 audit: the
existing 401→refresh→retry path in
`packages/cli/src/credentials-helper.ts:584-616` already routes refreshed
tokens to `dynamicCredentialsProxy.storeOAuthTokenDataIfNeeded` for
resolvable credentials. No re-wiring needed. The remaining race —
two concurrent executions for the same `(credentialId, subject)`
hitting the SaaS token endpoint simultaneously — is *the same* race that
exists for static OAuth2 credentials in upstream (no singleflight there
either) and only manifests for providers that rotate refresh tokens. The
DB write is atomic (composite PK + TypeORM upsert), so the worst case is
that one of the two concurrent calls sees a single transient error from
the provider; no data corruption. A proper fix would have to live in
`packages/core/src/execution-engine/.../request-helper-functions.ts`
(upstream territory) and is deferred until production telemetry shows the
race is actually painful.

**Upgrade checklist (rebase onto a new upstream)**

1. Confirm the `OAuthCredentialResolver` constructor signature in upstream
   matches `(logger, introspection, userinfo, jwt, storage, cipher)`. If
   upstream added or removed a dep, merge by hand — the test file lives
   in the same change-set.
2. Confirm the `OAuth2OptionsSchema` in
   `credential-resolvers/identifiers/oauth2-utils.ts` still exists and
   that adding a new optional field doesn't break unrelated identifiers.
   Re-run the resolver Jest suite (`oauth-credential-resolver.test.ts`).
3. The `RelayEventMap` additions are append-only — usually conflict-free,
   but a rename of the surrounding `oidc-graph-token-*` block would put
   the new event in the wrong neighborhood. Re-place by hand.
4. The `CacheService` injection in `DynamicCredentialService` assumes the
   service still lives at `@/services/cache/cache.service`. If upstream
   refactored the cache path (rare), update the import.
...
| --- | --- | --- |
| Admin creates a Monday OAuth2 resolver, users do per-user OAuth dance, workflows run as that user | **Supported** | The §10 machinery, exercised with a non-Microsoft provider |
| Same setup, optional `fallbackCredentialId` set to a shared service account | **Supported** | Per-user wins when present, falls back to shared on miss; audit event fires |
| Webhook trigger with `BearerTokenExtractor` configured, per-user token already seeded | **Supported** | Identifier extracts subject from the inbound bearer, resolver looks up keyed entry |
| Webhook trigger with no `BearerTokenExtractor` (machine webhook, anonymous chat) | **Supported via fallback** | Identifier returns no subject; resolver detects the failure, uses fallback if configured, throws otherwise |
| Workflow with 20+ nodes all using the same Monday credential | **Supported (optimized)** | PE-1/PE-2/PE-3 keep the per-node overhead at one cache get + one identifier call after the first node |
| Slack, Asana, Notion (any RFC 6749 authorization-code provider) | **Should work** — generic flow, not exercised live in this commit | Provider must expose either a discovery URL or you must populate `authorizeUrl`/`tokenUrl` directly on the credential type |
| Cross-tenant federation | **Not in scope** | Same caveat as §10 |
| App-only / client-credentials grant | **Not via the dance** | Use `POST /rest/credentials/:id/seed` from an external script, same as §9 |
...
**Upstream adoption (2.34.6) — frontend connect-gating dropped.** Upstream
2.34.6 shipped its own native per-user credential connect stack
("Quick Connect": `features/credentials/quickConnect/`, `features/resolvers/`,
`usePrivateCredentials`, `PrivateCredentialIcon`, and the `isResolvable` /
`connectedByMe` / `canConnect` concepts). The fork's earlier frontend
connect-gating delta (a `canConnectResolvableCredential` helper + a *second*
`QuickConnectButton` + re-gated OAuth banners across `CredentialCard.vue`,
`CredentialConfig.vue`, `CredentialEdit.vue`, `NodeCredentials.vue`) now
**duplicates** upstream and was **reverted to upstream on the 2.34.6 upgrade**;
`credential-permissions.utils.ts` was deleted. The fork's only real behavioural
delta was letting **read-only sharees** (`credential:read` without
`credential:connect`) self-connect on resolvable creds — but end-users are
onboarded via **credential sharing**, which grants
`['credential:read','credential:connect']` (`CREDENTIALS_SHARING_USER_SCOPES`),
so upstream's stricter `credential:connect` gate
(`CredentialConfig.vue::canConnect`) is **lossless** for the real flow.
**Do not re-introduce these frontend deltas on future rebases** — verify
`canConnectResolvableCredential` / a fork-added second `quick-connect-button`
stay absent (see the upstream-adoption audit). Backend resolver machinery
(§9–§12) is unaffected.

### 12. Dynamic credentials on manual/test runs + self-manual-reveal redaction

**What & why.** §9–§11 resolve per-user dynamic OAuth2 credentials on *live*
webhook triggers. §12 extends that so the credentials also resolve on **manual
"Execute workflow", test-webhook, and chat runs**, and so sub-nodes (e.g. AI
tools invoked via `supplyData`) inherit the resolution context via a
**supply-data execution-context fallback**. It also carries the **execution
...
- `packages/cli/src/modules/redaction/executions/execution-redaction.service.ts`
  — dynamic-credential executions are redacted for **everyone except**:
  1. the n8n user the credentials resolved to (`executedByUserId`), via
     `isOwnDynamicCredentialsExecution()`, **or**
  2. the user who **personally triggered a manual run**, via
     `isSelfManualReveal()` (matches `execution.data.manualData.userId ===
     requester.id`, manual modes only).
  Both are OR-combined in `canRevealDynamicCredentialsExecution(execution, user)`,
  which gates all three reveal paths. The `execution:reveal` scope deliberately
  does **not** bypass this — preserving the cross-tenant guarantee.
- `packages/cli/src/execution-lifecycle/execution-lifecycle-hooks.ts` —
  `hookFunctionsStampManualUser(hooks, executionMode, userId)` stamps
  `fullRunData.data.manualData.userId` on manual runs **before any save handler**,
  so the redaction match above has data to work with. Wired into
  `getLifecycleHooksForScalingWorker`, `getLifecycleHooksForScalingMain`, and
  `getLifecycleHooksForRegularMain`. (Queue + offload mode also pre-stamps in
  `workflow-execution.service.ts`; the hook is idempotent.)
- `packages/core/src/execution-engine/node-execution-context/*` — `_getCredentials`
  folds the execution-context fallback via `additionalData.executionContext`, so
  a zero-arg `getExecutionContext()` on load-options/supply-data contexts still
  resolves dynamic credentials for sub-nodes.

**⚠️ Rebase hazard (burned us on 2.34.6).** Upstream owns
`execution-redaction.service.ts`; the self-manual-reveal is a fork extension
layered on top. The 2.34.6 cherry-pick landed the **upstream** file and silently
dropped `canRevealDynamicCredentialsExecution` / `isSelfManualReveal` **and** the
matching test cases, and dropped `hookFunctionsStampManualUser` from the lifecycle
hooks — so the suite stayed green while the behavior was gone. On every rebase:
1. Confirm the three reveal gates call `canRevealDynamicCredentialsExecution`, not
   just `isOwnDynamicCredentialsExecution`.
2. Confirm `hookFunctionsStampManualUser` exists and is wired into all three
   main/worker lifecycle builders (before the save handlers).
3. Run `execution-redaction.service.test.ts` (self-manual-reveal cases) and
   `execution-lifecycle-hooks.test.ts` (asserts the stamp hook is registered and
   `manualData.userId` is stamped) — these encode the contract.

**Upstream adoption (2.34.6) — frontend publish-check dropped.** The fork
previously added a blanket `workflowHasIdentityProvidingTrigger()` early-return
in `useNodeHelpers.ts::collectPrivateCredentialIssues` to skip the private-cred
publish warning when a trigger supplies runtime identity. Upstream 2.34.6 now
does this natively and **more correctly**: `getBlockingTrigger` delegates to the
shared `classifyTriggerIdentity` (n8n-workflow) util — the *same* util the
backend §12 `validateDynamicCredentials`
(`workflow-validation.service.ts`) uses — so it is resolver-aware
(system vs custom) and FE↔BE consistent by construction. The fork's early-return
was redundant *and* buggy (it ignored the resolver/identity-type distinction and
short-circuited the correct check). It was **reverted to upstream on the 2.34.6
upgrade** (`useNodeHelpers.ts` + its test are now byte-identical to upstream).
**Do not re-introduce `workflowHasIdentityProvidingTrigger` on future rebases** —
the shared `classifyTriggerIdentity` util is the single source of truth for both
FE and BE. The backend redaction/lifecycle contract above is unaffected.

### 13. Webhook Node — multiple public keys for JWT auth

**What & why.** The **JWT Auth** credential can hold **several PEM public keys**;
an incoming webhook token is verified against each until one matches, so key
rotation works without swapping the credential. Single-key and passphrase
...
- `packages/nodes-base/credentials/JwtAuth.credentials.ts` — public-key field
  accepts one or more concatenated PEM blocks.
- `packages/nodes-base/nodes/Webhook/utils.ts` — `splitPemBlocks()` splits the
  configured keys and the verifier tries each in turn, **pinning
  `algorithms: [expectedAuth.algorithm]`** (no algorithm confusion) and returning
  403 on an empty key list or all-keys-fail.
- `packages/nodes-base/nodes/Webhook/test/utils.test.ts` — covers multi-key match
  and all-keys-fail paths.

**Rebase note.** Fork-only; if upstream reworks Webhook JWT verification,
re-apply the try-each-key loop and keep the algorithm pin. Security-reviewed on
2.34.6: no bypass via "any key matches" or alg confusion.
...
3. **Cherry-pick customizations in order.** From the current baseline branch
   ...
   **eleven** customization commits one at a time in this exact order (§8 was
   ...
   §12 build on §9/§10 and touch the most contended files.
   ...
   new upstream tag). Re-resolve them from the **previous** baseline branch
   rather than trusting this list verbatim:
   ...
   - After upgrade is verified end-to-end, fold each reconciliation hunk into
     the **latest feature commit that already touched that file** (`git commit
     --fixup` + `git rebase --autosquash`), leaving a single trailing
     `chore(upgrade-X.Y.Z)` for generated schema docs and cross-cutting drift.
     That keeps the next upgrade at one commit per customization. Verify the
     post-rebase tree is byte-identical to the pre-squash state
     (`git rev-parse HEAD^{tree}`).

   Pre-commit hooks cannot pass on replayed historical commits
   (`migration_timestamp_check`, biome, prettier). Bypass them during
   cherry-picks with `LEFTHOOK=0`; the full lint/format/typecheck gate later
   in this procedure is the real quality bar.
...
12. **Re-squash if needed.** If landing the upgrade required extra mechanical
    ...
    into the feature commit that already touched each file, leaving a single
    trailing `chore(upgrade-X.Y.Z)` for generated schema docs and cross-cutting
    drift, so the next upgrade still cherry-picks one commit per customization:
...
13. **Update this file.** Bump the baseline tags table to point to the new
    tag, update the per-section "Commit on current branch" SHAs and the SHAs in
    step 3, add a "Recorded findings" entry, and record any new
    upgrade-checklist lessons.
...
# NOTE: upstream renamed app/init.ts -> app/init/index.ts on 2.34.6; try both.
git show 'n8n@X.Y.Z:packages/frontend/editor-ui/src/app/init/index.ts' \
...
# §3 Login-event audit emits — BOTH are upstream-owned now (2.22.4 / 2.26.8).
# Expect NON-ZERO hits on the tag. Zero hits = upstream regressed; re-apply the
# fork shape recorded in §3. See "Audit / log-streaming login events".
git show 'n8n@X.Y.Z:packages/cli/src/controllers/auth.controller.ts' \
  | grep -nE 'validateSsoRestrictions|user-login-failed'
git show 'n8n@X.Y.Z:packages/cli/src/modules/sso-oidc/oidc.controller.ee.ts' \
  | grep -nE "emit\('user-logged-in'"
# …and after the §3 cherry-pick, assert EXACTLY ONE emit site on HEAD.
# More than one = the retired fork emit came back and every login double-counts.
git grep -c "emit('user-logged-in'" HEAD \
  -- packages/cli/src/modules/sso-oidc/oidc.controller.ee.ts   # expect 1
# Same file, same hazard: assert the DI param wasn't doubled by auto-merge.
git show HEAD:packages/cli/src/modules/sso-oidc/oidc.controller.ee.ts \
  | grep -c 'private readonly eventService'                     # expect 1

# §3 Azure AD roles-array — DROPPED on 2.34.6, superseded by expression mapping.
# Upstream expression mapping must be present (non-zero), fork array branch gone (0).
git grep -nE 'scopesUseExpressionMapping|provisionExpressionMappedRolesForUser' \
  'n8n@X.Y.Z' -- packages/cli/src/modules/provisioning.ee/provisioning.service.ee.ts
git grep -nE 'Array\.isArray\(roleSlugInput\)' \
  HEAD -- packages/cli/src/modules/provisioning.ee/   # expect 0 hits

# §10 migration slot — the fork migration timestamp must not collide upstream,
# and it must be registered in BOTH index files in chronological position.
git ls-tree -r --name-only 'n8n@X.Y.Z' -- packages/@n8n/db/src/migrations/common/ \
  | grep 1785500832627   # expect 0 hits (slot still free)
git grep -n 'AddOidcSeedSourceToCredentialResolver' HEAD \
  -- packages/@n8n/db/src/migrations/sqlite/index.ts \
     packages/@n8n/db/src/migrations/postgresdb/index.ts   # expect 2 hits each file
...
# §12 dyncred self-manual-reveal — MUST survive every rebase (silently dropped on 2.34.6).
# Upstream owns execution-redaction.service.ts; our self-reveal is layered on top.
git grep -nE 'canRevealDynamicCredentialsExecution|isSelfManualReveal' \
  HEAD -- packages/cli/src/modules/redaction/executions/execution-redaction.service.ts
git grep -nE 'hookFunctionsStampManualUser' \
  HEAD -- packages/cli/src/execution-lifecycle/execution-lifecycle-hooks.ts
# If either returns 0 hits after a rebase, re-apply from the previous baseline tag
# and re-run execution-redaction.service.test.ts + execution-lifecycle-hooks.test.ts.

# §13 Webhook multi-key JWT — fork-only
git grep -nE 'splitPemBlocks' \
  'n8n@X.Y.Z' -- packages/nodes-base/nodes/Webhook/utils.ts

# §11/§12 FE — connect-gating + publish-check were DEFERRED to upstream on 2.34.6.
# These fork symbols must stay GONE. Any hit on HEAD = a rebase re-introduced dropped
# fork code that now duplicates upstream's native Quick Connect / classifyTriggerIdentity.
git grep -nE 'canConnectResolvableCredential|workflowHasIdentityProvidingTrigger' \
  HEAD -- packages/frontend/editor-ui/src   # expect 0 hits
test ! -e packages/frontend/editor-ui/src/features/credentials/credential-permissions.utils.ts \
  && echo "OK: utils file absent" || echo "WARN: fork helper re-introduced"
```
...
**Recorded findings (2.26.8, against `n8n@2.26.8` = `074835ea87a`).**
The dynamic-credentials areas (§9, §10, §11) joined the cherry-pick set here,
taking the run to 11 commits. Two adoptions and one collision:

- §3 OIDC — **significant partial adoption.** Upstream now ships
  `scopesInstanceRoleClaimName` in `sso.config.ts`, the single-claim lookup in
  `oidc.service.ee.ts`, **and** the `user-logged-in` emit in
  `oidc.controller.ee.ts` (fork-only at 2.22.4). The fork emit was dropped;
  the documented `EventService` double-DI auto-merge fired again and was
  deduped. Keep the fork superset for project-role handling, the diagnostic
  fingerprint, and the access-token fallback.
- §10 — **first migration-timestamp collision.** The fork's
  `…022-AddOidcSeedSourceToCredentialResolver` collided with upstream's new
  common migrations; renumbered to `…029` (file `git mv` + class rename +
  both index files). The migration is idempotent, so renumbering is safe on
  existing DBs. This became a recurring per-upgrade check.
- §11 redaction — **dropped, superseded upstream.** Upstream independently
  implemented dynamic-credential redaction via `executedByUserId` /
  `isOwnDynamicCredentialsExecution`, which covers manual *and*
  chat-hub/production; the fork's manual-only `isSelfManualReveal` variant was
  a strict subset at that point. All §11 redaction hunks were dropped in
  favour of upstream. (Note: the fork later re-layered a genuine self-reveal
  extension on top — see §12 and the 2.30.6 findings.)
- §5 Docker — **largely upstream already.** Base image, alpine pin, and the
  `sqlite3` / `isolated-vm` rebuild split had all landed upstream; only the
  scratch-flatten stage remained fork-specific.

**Recorded findings (2.30.6, against `n8n@2.30.6` = `a5d97843e15`).**
Process changes rather than new collisions. This upgrade introduced the
**review gates** (`code-reviewer-pro` after each cherry-pick phase,
`security-auditor` after reconciliation, `ci-investigator` after the test gate)
and the **pack + dockerize + smoke** phase, both of which are now part of the
standard procedure. Substantively: the fork's self-manual-reveal was
**re-applied on top of** upstream's redaction (upstream covers the resolved
`executedByUserId`; the fork additionally reveals to whoever *personally
triggered* a manual run), together with the `manualData.userId` stamp and the
`getCredentialScopes` global-read grant. That pairing is what later became §12.

**Recorded findings (2.32.6, against `n8n@2.32.6` = `c580585cf48`).**
§12 was promoted from loose reconciliation into its own catalogue section and
commit, taking the run to 12. Main lesson came from the CI gate: **W9R
initially FAILED** on ~22 editor-ui §11 connect-gating assertion failures that
had been misclassified as upstream drift. They were real. Resolution pattern
worth repeating — when a cluster of same-area test failures appears, diff each
failing file against the pristine upstream tag *before* labelling it
environmental; byte-identical to upstream ⇒ not fork-introduced, anything else
⇒ investigate.

**Recorded findings (2.34.6, against `n8n@2.34.6` = `d743b0ad2e0`).**
The largest jump so far. §13 added; run reached 13 commits. Four findings that
changed standing policy:

- §3 — **Azure AD `roles`-array normalization DROPPED** in favour of upstream
  expression-based role mapping. See the note at the top of §3; this is settled
  and re-verified on 2.35.7.
- §11/§12 frontend — **connect-gating and the publish-check DEFERRED to
  upstream.** Upstream shipped native per-user credential connect ("Quick
  Connect") plus a resolver-aware publish-time check through the shared
  `classifyTriggerIdentity` util (the same one the backend §12
  `validateDynamicCredentials` uses). The fork's only real delta was letting
  read-only sharees self-connect — lossless in practice, because end users are
  onboarded via credential sharing, which grants
  `['credential:read','credential:connect']`. `canConnectResolvableCredential`
  and `workflowHasIdentityProvidingTrigger` were reverted to upstream and must
  stay gone.
- §12 — **silent behavioural drop caught only at the review gate.** The
  cherry-pick landed upstream's `execution-redaction.service.ts` without the
  fork's self-manual-reveal, dropped `hookFunctionsStampManualUser`, *and*
  removed the matching test cases — so build, typecheck, lint, and tests were
  all green while the behaviour was gone. This is why §12 now carries an
  explicit rebase-hazard block and why the adoption-audit greps `HEAD` (not
  just the upstream tag) for those symbols.
- **Turbo cache masked 24 real cli errors.** `pnpm build` and `pnpm typecheck`
  both reported green off cache hits; a direct `tsc --noEmit` surfaced the real
  failures. **A clean rebuild (`pnpm reset`) is now mandatory before trusting
  any gate.** Related drift classes seen: jest→vitest remnants in fork test
  files (upstream migrated cli tests off Jest in this window), dynamic
  `import()` calls missing the `.js` suffix, constructor-arg drift in service
  tests, and duplicate `projectId` keys where the fork's `project_id` metric
  label met upstream's new `source` field.

**Recorded findings (2.35.7, against `n8n@2.35.7`).** Pre-flight audit
(2026-08-23), before cherry-picking:

- **All 13 customizations still required** — every fork marker returns 0 hits
  on the tag (`nodeGovernance`, `akeyless`, `splitPemBlocks`,
  `canRevealDynamicCredentialsExecution`, `hookFunctionsStampManualUser`,
  `resolveInstanceRoleClaim`, `decodeJwtPayloadUnsafe`, `includeProjectIdLabel`,
  `workflowProjectCache`, `useApim`). Nothing new was adopted upstream.
- **Contained overlap.** Upstream delta is large (283 commits, 2309 files,
  +104136/-31424) but only **85 of the fork's 338 touched files** also churned
  upstream. Hotspots by churn: the two migration index files, `en.json`,
  `workflow.service.test.ts`, `test-webhooks.test.ts`, `workflow.service.ts`,
  `webhook-helpers.ts`.
- **§10 migration slot `1785500832627` is still free** — upstream goes
...
- **§5 Docker is a version bump.** `NODE_VERSION` 24.18.0 → 24.18.1 with
  ...
  scratch-flatten, `JOBS=1 npm rebuild isolated-vm`, and the `dockerize-n8n.mjs`
  proxy build-args layer on top unchanged.
...
- [ ] OIDC "Test Connection" button on SSO settings succeeds. The
      userinfo→ID-token-claims fallback now covers this path too (§3
      "Userinfo fallback"), so the old cosmetic asymmetry is gone. A
      `BadRequestError: Invalid token` here **is** now a regression —
      most likely the fallback was dropped in a re-squash again.
