...
export class NodeGovernanceService {
	...
	async getGovernanceForNodes(
		nodeTypes: string[],
		projectId: string,
		// Undefined for non-interactive executions (schedule/webhook/trigger). Policy
		// resolution never depends on the user; only pending-request status does.
		...
		// Get pending requests for this user in this project. Skipped without a user:
		// a pending request is a property of a requester, and it downgrades `blocked`
		// to `pending_request`, which callers still treat as blocking either way.
	...
	async validateWorkflowNodes(
		...
		projectId: string,
		// Undefined for non-interactive executions; enforcement still applies.
		userId: string | undefined,
		entityManager?: EntityManager,
	): Promise<{
